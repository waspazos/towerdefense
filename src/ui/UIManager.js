...
import { roundConfig } from '../config/roundConfig';
...
export class UIManager {
  constructor() {
    ...
    eventSystem.on('showUI', this.showUI.bind(this));
    eventSystem.on('hideUI', this.hideUI.bind(this));
    eventSystem.on('updateUI', this.updateUI.bind(this));
    eventSystem.on('goldChanged', this.updateGold.bind(this));
    eventSystem.on('kingHealthChanged', this.updateKingHealth.bind(this));
    eventSystem.on('towerCountChanged', this.updateTowerCount.bind(this));
    eventSystem.on('roundStarted', this.updateRoundCounter.bind(this));
    eventSystem.on('roundCompleted', this.updateRoundCounter.bind(this));
    eventSystem.on('interRoundTimerUpdated', this.updateRoundTimer.bind(this));
    eventSystem.on('towerDetailsUpdated', this.updateTowerDetails.bind(this));
    eventSystem.on('showAugmentSelection', this.showAugmentSelection.bind(this));
    eventSystem.on('augmentSelected', this.updateAugmentTracker.bind(this));
    eventSystem.on('workerHired', this.updateWorkerList.bind(this));
  }
  ...
  updateGold(data) {
    ...
    if (data && data.gold !== undefined) {
      ...
      eventSystem.emit('getGold', { callback: (value) => { gold = value; } });
    }
  ...
  updateTowerCount(data) {
    ...
    if (data && data.count !== undefined) {
      ...
      eventSystem.emit('getTowerCount', { callback: (value) => { count = value; } });
    }
  ...
  updateRoundCounter(data) {
    ...
    if (data) {
      ...
      eventSystem.emit('getCurrentRound', { callback: (value) => { round = value; } });
    }
    
    eventSystem.emit('getMaxRounds', { callback: (value) => { maxRounds = value; } });
  ...
  updateRoundTimer(data) {
    ...
    eventSystem.emit('isRoundActive', { callback: (value) => { roundActive = value; } });
    ...
    if (roundActive) {
      ...
      if (data && data.timer !== undefined) {
        ...
        eventSystem.emit('getInterRoundTimer', { callback: (value) => { timer = value; } });
      }
  ...
  updateRoundTracker(currentRound) {
    ...
    if (currentRound === undefined) {
      eventSystem.emit('getCurrentRound', { callback: (value) => { currentRound = value; } });
    }
    ...
    if (previousRound >= 1) {
      previousRoundElem.textContent = previousRound;
  ...
  showAugmentSelection(data) {
    ...
    augments.forEach(augment => {
      ...
      option.addEventListener('click', () => {
        eventSystem.emit('selectAugment', { augment });
  ...
  updateAugmentTracker() {
    ...
    // Get active augments
    let activeAugments = [];
    eventSystem.emit('getActiveAugments', { callback: (augments) => { activeAugments = augments; } });
  ...
  updateWorkerList() {
    ...
    // Get workers
    let workers = [];
    eventSystem.emit('getWorkers', { callback: (workerList) => { workers = workerList; } });
