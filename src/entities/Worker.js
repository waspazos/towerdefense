...
export class Worker extends Entity {
  constructor(position) {
    const mesh = window.game.renderer.createWorkerMesh();
    super(position, mesh);
  ...
  updateMovement(delta) {
    ...
    if (distanceToTarget < 0.2) {
      ...
      window.game.eventSystem.emit("workerReachedDestination", { worker: this });
    }
  }
  ...
  updateMining(delta) {
    ...
    if (this.miningTimer >= this.miningInterval) {
      ...
      // Generate gold
      window.game.eventSystem.emit("workerMinedGold", {
        worker: this,
  ...
  returnToCamp() {
    ...
    this.moveToPosition(new window['THREE'].Vector3(campPosition.x, 0, campPosition.z));
    this.targetRock = null;
  }
