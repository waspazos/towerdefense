...
export class Creep extends Entity {
  constructor(type, pathIndex, difficulty = 1) {
    ...
    // Get path
    let path = null;
    eventSystem.emit('getPath', { pathIndex, callback: (result) => { path = result; }});
    ...
    // Create mesh
    const mesh = renderer.createCreepMesh(type);
    
    // Initialize entity with position at spawn point
    const position = new THREE.Vector3(path.spawnPoint.x, 0.5, path.spawnPoint.z);
    super(position, mesh);
  ...
  reachEnd() {
    if (!this.reachedKing) {
      ...
      eventSystem.emit('creepReachedEnd', { 
        creep: this,
  ...
  addSlowVisualEffect() {
    this.slowEffectVisual = renderer.createSlowEffect(this.mesh);
  }
  
  removeSlowVisualEffect() {
    if (this.slowEffectVisual) {
      renderer.removeSlowEffect(this.slowEffectVisual);
      this.slowEffectVisual = null;
    }
  }
  ...
  addBurnVisualEffect() {
    this.burnEffectVisual = renderer.createBurnEffect(this.mesh);
  }
  
  removeBurnVisualEffect() {
    if (this.burnEffectVisual) {
      renderer.removeBurnEffect(this.burnEffectVisual);
