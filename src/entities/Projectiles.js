...
class Projectile extends window.Entity {
  constructor(options) {
    ...
    const mesh = window.renderer.createProjectileMesh(color);
    super(position, mesh);
  ...
  hitTarget() {
    ...
    if (!killed) {
      if (this.towerType === 'frost' && this.target.applySlowEffect) {
        ...
        const slowAmount = window.towerConfig.frost.ranks[this.towerRank - 1].slowAmount;
  ...
  createFrostImpactEffect() {
    window.eventSystem.emit('createFrostImpact', { 
      position: this.position.clone()
    });
  }
  
  createFireCriticalEffect() {
    window.eventSystem.emit('createFireCritical', { 
      position: this.position.clone()
    });
  }
  
  createBasicImpactEffect() {
    window.eventSystem.emit('createBasicImpact', { 
      position: this.position.clone()
    });
  }
}

// Make Projectile class globally available
